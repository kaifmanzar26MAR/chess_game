...
const sendMessage = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json(new ApiResponse(200, {conversation}, "Message saved saved successfully!!"));
});
...
const getMessages = asyncHandler(async(req, res)=>{
    ...
    if(!conversation){
        const messages = [];
        return res.status(200).json(new ApiResponse(200, {messages}, "No Conversation Found!!"));
    }

    const all_message_ids = conversation.messageIds;
    const all_message = await Message.find({ _id: { $in: all_message_ids } });

    return res.status(200).json(new ApiResponse(200, {all_message}, "Conversation got Successfully!!"));
})
